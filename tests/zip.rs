...
mod tests {
    ...
    fn zipped_stride_iters() {
        let matrices = [1i16, 2, 3, 4, 5, 6, 7, 8, 9][..]
            .iter()
            .cycle()
            .take(9 * 100)
            .map(|i| i.clone())
            .collect::<Vec<_>>();
        let determinants = (&matrices[..])
            .stride_nine(tuplify!(9, i16s(0)))
            .zip()
        ...
        let matrices = [1i64, 0, 0, 0, 5, 4, 2, 3, 0][..]
            .iter()
            .cycle()
            .take(9 * 100)
            .map(|i| i.clone())
        ...
        assert!(determinants
            .iter()
            .fold(true, |acc, x| { acc && x == &-12 }));
    }

    #[test]
    #[cfg(feature = "std")]
    fn zipped_heterogeneous_iters() {
        let to_stride = [1i8, 2, 3, 4, 5, 6, 7, 8][..]
            .iter()
            .cycle()
            .take(512)
            .map(|i| i.clone())
        ...
        let a_times_three = (a, standard_iter_a)
            .zip()
            .simd_map(|(s, c)| s * c)
            .scalar_collect();

        let b_times_three = (b, standard_iter_b)
            .zip()
            .simd_map(|(s, c)| s * c)
            .scalar_collect();
        ...
        assert!(a_times_three_check
            .zip(a_times_three)
            .fold(true, |acc, (a, b)| acc && a == b));

        assert!(b_times_three_check
            .zip(b_times_three)
            .fold(true, |acc, (a, b)| acc && a == b));
    }
    ...
    fn zip_simd_reduce() {
        let vec1 = [2u32; 129];
        let vec2 = [3u32; 129];
        let result = (vec1.simd_iter(u32s(0u32)), vec2.simd_iter(u32s(0u32)))
            .zip()
            .simd_reduce(u32s(0u32), |acc, (x, y)| acc + x * y)
            .sum();
