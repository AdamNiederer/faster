        ...
        ($name:ident, $native_type:ty, $simd_type:ident) => {
            ...
            fn $name() {
                for n in 0..16 {
                    ...
                    let sum: $native_type = (
                        (&vec_of_3[..]).simd_iter($simd_type(0)),
                        (&vec_of_1[..]).simd_iter($simd_type(0)),
                    )
                        .zip()
        ...
        ($name:ident, $native_type:ty, $simd_type:ident) => {
            ...
            fn $name() {
                for n in 0..16 {
                    ...
                    let sum_scalar: $native_type = vec_of_1
                        .iter()
                        .zip(vec_of_3.iter())
                        .map(|(a, b)| (a - b) * (a - b))
                        .sum();
                    ...
                    // Same as above, but this time we reduce with simd_reduce
                    let sum_simd: $native_type = (
                        vec_of_1.simd_iter($simd_type(0.0 as $native_type)),
