    ...
    fn upcast(self) -> (u16x16, u16x16) {
        ...
        unsafe {
            (
                ...
                _mm256_cvtepu8_epi16(transmute(_mm256_castsi256_si128(transmute(
                    _mm256_permute4x64_epi64(self.be_i64s(), 0x0E).be_u16s(),
                ))))
                .be_u16s(),
            )
        }
...
impl Upcast<i16x16> for i8x32 {
    ...
    fn upcast(self) -> (i16x16, i16x16) {
        ...
        unsafe {
            (
                _mm256_cvtepi8_epi16(transmute(_mm256_castsi256_si128(transmute(self)))),
                _mm256_cvtepi8_epi16(transmute(_mm256_castsi256_si128(transmute(
    ...
    fn upcast(self) -> (u32x8, u32x8) {
        ...
        unsafe {
            (
                ...
                _mm256_cvtepu16_epi32(transmute(_mm256_castsi256_si128(transmute(
                    _mm256_permute4x64_epi64(self.be_i64s(), 0x0E).be_u32s(),
                ))))
                .be_u32s(),
            )
        }
...
impl Upcast<i32x8> for i16x16 {
    ...
    fn upcast(self) -> (i32x8, i32x8) {
        ...
        unsafe {
            (
                _mm256_cvtepi16_epi32(transmute(_mm256_castsi256_si128(transmute(self)))),
                _mm256_cvtepi16_epi32(transmute(_mm256_castsi256_si128(transmute(
                    _mm256_permute4x64_epi64(self.be_i64s(), 0x0E),
                )))),
            )
        }
...
impl Upcast<f64x2> for f32x4 {
    ...
    fn upcast(self) -> (f64x2, f64x2) {
        ...
        unsafe {
            (
                _mm_cvtps_pd(self),
                _mm_cvtps_pd(_mm_shuffle_epi32(self.be_i32s(), 0x0E).be_f32s_unchecked()),
            )
        }
    }
    ...
    fn upcast(self) -> (f64x2, f64x2) {
        fallback!();
        (
            f64x2::new(self.extract(0) as f64, self.extract(1) as f64),
            f64x2::new(self.extract(2) as f64, self.extract(3) as f64),
        )
    }
}
...
impl Upcast<f64x2> for i32x4 {
    ...
    fn upcast(self) -> (f64x2, f64x2) {
        ...
        unsafe {
            (
                _mm_cvtepi32_pd(self),
                _mm_cvtepi32_pd(_mm_shuffle_epi32(self, 0x0E)),
            )
        }
    }
...
impl Upcast<i64x2> for i32x4 {
    ...
    fn upcast(self) -> (i64x2, i64x2) {
        fallback!();
        (
            i64x2::new(self.extract(0) as i64, self.extract(1) as i64),
            i64x2::new(self.extract(2) as i64, self.extract(3) as i64),
        )
    }
}
...
impl Upcast<u64x2> for u32x4 {
    ...
    fn upcast(self) -> (u64x2, u64x2) {
        fallback!();
        (
            u64x2::new(self.extract(0) as u64, self.extract(1) as u64),
            u64x2::new(self.extract(2) as u64, self.extract(3) as u64),
        )
    }
}
...
impl Upcast<f64x4> for f32x8 {
    ...
    fn upcast(self) -> (f64x4, f64x4) {
        ...
        unsafe {
            (
                _mm256_cvtps_pd(transmute(_mm256_castsi256_si128(transmute(self)))),
                _mm256_cvtps_pd(transmute(_mm256_castsi256_si128(transmute(
                    _mm256_permute4x64_epi64(self.be_i64s(), 0x0E),
                )))),
            )
        }
...
impl Upcast<f64x4> for i32x8 {
    ...
    fn upcast(self) -> (f64x4, f64x4) {
        ...
        unsafe {
            (
                _mm256_cvtepi32_pd(transmute(_mm256_castsi256_si128(transmute(self)))),
                _mm256_cvtepi32_pd(transmute(_mm256_castsi256_si128(transmute(
                    _mm256_permute4x64_epi64(self.be_i64s(), 0x0E),
                )))),
            )
        }
...
impl Upcast<i64x4> for i32x8 {
    ...
    fn upcast(self) -> (i64x4, i64x4) {
        ...
        unsafe {
            (
                _mm256_cvtepi32_epi64(transmute(_mm256_castsi256_si128(transmute(self)))),
                _mm256_cvtepi32_epi64(transmute(_mm256_castsi256_si128(transmute(
                    _mm256_permute4x64_epi64(self.be_i64s(), 0x0E),
                )))),
            )
        }
    ...
    fn upcast(self) -> (u64x4, u64x4) {
        ...
        unsafe {
            (
                ...
                _mm256_cvtepu32_epi64(transmute(_mm256_castsi256_si128(transmute(
                    _mm256_permute4x64_epi64(transmute(self), 0x0E).be_u32s(),
                ))))
                .be_u64s(),
            )
        }
