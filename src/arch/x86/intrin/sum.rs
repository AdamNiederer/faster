...
impl Sum for i8x16 {
    ...
    fn sum(&self) -> Self::Scalar {
        optimized!();
        let pos =
            unsafe { _mm_sad_epu8(self.max(Self::splat(0)).be_u8s(), u8x16::splat(0)).be_u16s() };
        let neg = unsafe {
            _mm_sad_epu8(self.min(Self::splat(0)).abs().be_u8s(), u8x16::splat(0)).be_u16s()
        };
        pos.extract(0)
            .overflowing_sub(neg.extract(0))
            .0
            .overflowing_add(pos.extract(4).overflowing_sub(neg.extract(4)).0)
            .0 as i8
    }
}
...
impl Sum for i8x32 {
    ...
    fn sum(&self) -> Self::Scalar {
        optimized!();
        let pos = unsafe {
            _mm256_sad_epu8(self.max(Self::splat(0)).be_u8s(), u8x32::splat(0)).be_u16s()
        };
        let neg = unsafe {
            _mm256_sad_epu8(self.min(Self::splat(0)).abs().be_u8s(), u8x32::splat(0)).be_u16s()
        };
        pos.extract(0)
            .overflowing_sub(neg.extract(0))
            .0
            .overflowing_add(pos.extract(4).overflowing_sub(neg.extract(4)).0)
            .0
            .overflowing_add(pos.extract(8).overflowing_sub(neg.extract(8)).0)
            .0
            .overflowing_add(pos.extract(12).overflowing_sub(neg.extract(12)).0)
            .0 as i8
    }
}
...
impl Sum for i16x8 {
    ...
    fn sum(&self) -> Self::Scalar {
        ...
        let x = unsafe {
            _mm_hadd_epi16(
                _mm_hadd_epi16(_mm_hadd_epi16(*self, Self::splat(0)), Self::splat(0)),
                Self::splat(0),
            )
        };
        x.extract(0)
...
impl Sum for i16x16 {
    ...
    fn sum(&self) -> Self::Scalar {
        ...
        let x = unsafe {
            _mm256_hadd_epi16(
                _mm256_hadd_epi16(_mm256_hadd_epi16(*self, Self::splat(0)), Self::splat(0)),
                Self::splat(0),
            )
        };
    ...
    fn sum_upcast(&self) -> i64 {
        ...
        unsafe {
            ...
            let x = _mm256_hadd_epi32(
                _mm256_hadd_epi32(a.be_i32s(), i32x8::splat(0)),
                i32x8::splat(0),
            );
            let y = _mm256_hadd_epi32(
                _mm256_hadd_epi32(b.be_i32s(), i32x8::splat(0)),
                i32x8::splat(0),
            );
...
impl Sum for i32x8 {
    ...
    fn sum(&self) -> Self::Scalar {
        optimized!();
        let x =
            unsafe { _mm256_hadd_epi32(_mm256_hadd_epi32(*self, Self::splat(0)), Self::splat(0)) };
...
impl_packed_sum!(
    u8x64, i8x64, u16x32, u16x8, i16x32, u32x16, u32x8, u32x4, i32x16, i32x4, f32x16, u64x8, u64x4,
    u64x2, i64x8, i64x4, i64x2, f64x8
);
impl_packed_upcast_sum!(
    u8x64, i8x64, u16x32, u16x8, i16x32, i16x8, u32x16, u32x8, u32x4, i32x16, i32x8, i32x4, f32x16,
    f32x8, f32x4, u64x8, u64x4, u64x2, i64x8, i64x4, i64x2, f64x8, f64x4, f64x2
);
