...
rust_fallback_impl! {
    impl PackedRound for f32x4 where "sse4.1" {
        round => _mm_round_ps(_MM_FROUND_TO_NEAERST_INT), [0, 1, 2, 3];
...
rust_fallback_impl! {
    impl PackedRound for f64x2 where "sse4.1" {
        round => _mm_round_pd(_MM_FROUND_TO_NEAERST_INT), [0, 1];
...
rust_fallback_impl! {
    impl PackedRound for f32x8 where "avx" {
        round => _mm256_round_ps(_MM_FROUND_TO_NEAERST_INT), [0, 1, 2, 3, 4, 5, 6, 7];
...
rust_fallback_impl! {
    impl PackedRound for f64x4 where "sse4.1" {
        round => _mm256_round_pd(_MM_FROUND_TO_NEAERST_INT), [0, 1, 2, 3];
